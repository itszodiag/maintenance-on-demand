...
export function CompanyTechniciansPage() {
  ...
  useEffect(() => {
    setHeader('Technician Management', 'Grow your technician network and keep the company team organized.');
    loadPage();
  }, [setHeader]);
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <>
          <div className="rounded-[28px] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 p-6 shadow-sm">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <input
                ...
                className="rounded-[18px] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-4 py-2.5 text-sm placeholder-slate-400 dark:placeholder-slate-500 focus:border-blue-500 dark:focus:border-blue-400 focus:outline-none"
              />
            </div>
            ...
            <div className="mt-6 space-y-3">
              {filteredTeam.length === 0 ? (
                ...
                filteredTeam.map((entry) => (
                  <div
                    key={entry.id}
                    className="flex flex-col gap-3 rounded-[24px] border border-slate-100 bg-slate-50 dark:bg-slate-900/80 p-4 sm:flex-row sm:items-center sm:justify-between"
                  >
                    <div>
                      ...
                      <div className="mt-2 flex gap-2">
                        <span className="rounded-full bg-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-700 dark:text-slate-300">
              ...
              {discoverable.map((technician) => (
                ...
                >
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <span className="rounded-full bg-white dark:bg-slate-950 px-3 py-1 text-xs font-semibold text-slate-600 dark:text-slate-400">
...
function EmptyBlock({ message }) {
  return (
    <div className="rounded-[28px] border border-dashed border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 px-5 py-10 text-center text-sm text-slate-500 dark:text-slate-400">
      {message}
    </div>
  );
}
...
function InviteTechnicianModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg rounded-[30px] border border-white/70 dark:border-slate-800/70 bg-white/95 dark:bg-slate-950/95 p-8 shadow-[0_35px_80px_-45px_rgba(37,99,235,0.55)] backdrop-blur-sm">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-full p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800 hover:text-slate-600 dark:text-slate-400"
          >
            <X className="h-5 w-5" />
          </button>
        </div>

        <div className="mb-6 rounded-[24px] bg-slate-50 dark:bg-slate-900 p-4">
          <p className="font-semibold text-slate-900 dark:text-slate-100">
            {technician.display_name || technician.name}
          </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <textarea
              ...
              className="w-full rounded-lg border border-slate-300 dark:border-slate-700 px-3 py-2 focus:border-blue-500 dark:focus:border-blue-400 focus:outline-none"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="rounded-full px-6 py-2 text-sm font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800"
            >
              Cancel
            </button>
            <button
